...
public class BodegueroDAO {
    ...
    public ResultSet listarProductosStockBajo() {
        try {
            String sql = """
                    SELECT
                        p.id_producto,
    ...
    public boolean actualizarStock(int idProducto, int cantidad, boolean esEntrada) {
        try {
    ...
    public boolean insertarProducto(String codigo, String nombre, String descripcion,
                                    double precio, int stock, int estado, int categoria) {
        try {
            String sql = """
                    INSERT INTO productos (codigo, nombre, descripcion, precio, stock, estado, id_categoria)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                    """;
            PreparedStatement ps = conexion.prepareStatement(sql);
    ...
    public boolean editarProducto(int id, String codigo, String nombre, String descripcion,
                                  double precio, int stock, int estado, int categoria) {
        try {
            String sql = """
                    UPDATE productos
                    SET codigo=?, nombre=?, descripcion=?, precio=?, stock=?, estado=?, id_categoria=?
                    WHERE id_producto=?
                    """;
            PreparedStatement ps = conexion.prepareStatement(sql);
            ps.setString(1, codigo);
